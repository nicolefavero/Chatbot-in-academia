...
IMPORTANT: Extract entities specifically mentioned in THIS text. Do not copy entities from the examples unless they are actually present in the current text.
...
def extract_elements_from_chunk(chunk, tokenizer, model):
    ...
    # MODIFIED: Changed entity types from ORGANIZATION, PERSON, GEO to academic-focused types
    ...
    debug("RAW OUTPUT SAMPLE (first 200 chars): " + text[:200])
    ...
    try:
        ...
        json_pattern = r'\[\s*\{\s*"name":.+\}\s*\]'
        matches = re.search(json_pattern, text, re.DOTALL)
        if matches:
            try:
                data = json.loads(matches.group(0))
            except:
                debug("Second JSON parsing attempt failed")
                data = {"entities": [], "relationships": []}
        else:
            debug("Could not find JSON array in response")
            data = {"entities": [], "relationships": []}
    ...
    debug(f"Extracted {len(data.get('entities', []))} entities and {len(data.get('relationships', []))} relationships")
    return data

def extract_graph_elements(all_chunks, summ_tokenizer, summ_model, max_chunks=None):
    ...
    for doc_idx, chunks in enumerate(all_chunks):
        ...
        # FIXED: Limit processing to max_chunks for testing if specified
    ...
    # FIXED: Add validation to check for diverse entity extraction
...
def filter_relevant_communities_improved(question, community_summaries, sent_transformer):
    debug("Filtering relevant communities with improved approach.")
    ...
    # Detect if it's a general question about dataset topics
    ...
    is_general_question = any(pattern in question.lower() for pattern in general_question_patterns)
    
    selected_communities = {}
    if is_general_question:
        # For general questions, take the top scoring communities
        top_n = min(5, len(community_summaries))
        top_indices = np.argsort(sim_scores)[-top_n:]
        for idx in top_indices:
            cid = list(community_summaries.keys())[idx]
            selected_communities[cid] = community_summaries[cid]
    else:
        # For specific questions, use a threshold but with a fallback
        for idx, (cid, summary) in enumerate(community_summaries.items()):
            if sim_scores[idx] > 0.35:  # Original threshold
                selected_communities[cid] = summary
        
        # Fallback: If no communities were selected, take the top 3
        if not selected_communities:
            top_3 = min(3, len(community_summaries))
            top_indices = np.argsort(sim_scores)[-top_3:]
            for idx in top_indices:
                cid = list(community_summaries.keys())[idx]
                selected_communities[cid] = community_summaries[cid]
...
def create_metadata_community(community_summaries, G):
    """Create a metadata community with high-level information about the dataset"""
    debug("Creating metadata community for dataset overview.")
    
    # Get top entities by centrality
    centrality = nx.degree_centrality(G)
    top_entities = sorted(centrality.items(), key=lambda x: x[1], reverse=True)[:15]
    ...
    # Build metadata text
    metadata_text = f"""
    This dataset contains information from a knowledge graph with {G.number_of_nodes()} entities and {G.number_of_edges()} relationships.
    The graph is organized into {len(community_summaries)} topic communities.
    
    Top entities in the dataset (by centrality):
    {', '.join([entity for entity, _ in top_entities])}
    
    Entity types in the dataset:
    {', '.join([f"{t} ({c})" for t, c in sorted(entity_types.items(), key=lambda x: x[1], reverse=True)])}
    
    Major topics include the communities of these entities and their relationships.
    """
    
    # Add as special community
    metadata_community_id = "metadata"
    community_summaries[metadata_community_id] = metadata_text
    
    debug("Metadata community created.")
    return community_summaries

# ADDED: New function to calculate weighted centrality for academic concepts
def calculate_weighted_centrality(G):
    """Calculate centrality with higher weights for academic concept types"""
    debug("Calculating weighted centrality for academic concepts")
    
    # Give higher weights to research concepts than organizations/people
    type_weights = {
...
def graph_rag_query_map_reduce_improved(question, community_summaries, summ_tokenizer, summ_model, sent_transformer, G=None):
    debug("Starting improved map-reduce for query.")
    
    # Improved detection for top topics questions
    question_lower = question.lower()
    
    # Check for keywords related to "top" and "topics"
    top_words = ["top", "main", "key", "primary", "important"]
    topic_words = ["topic", "topics", "entity", "entities", "subject", "theme"]
    
    is_top_topics_question = False
    if any(word in question_lower for word in top_words) and any(word in question_lower for word in topic_words):
        is_top_topics_question = True
        debug(f"Detected top topics question: {is_top_topics_question}")
    
    # Direct answer for top topics questions
    if is_top_topics_question and G is not None:
        debug("Generating direct answer for top topics question.")
        
        # MODIFIED: Using weighted centrality for academic concepts
        centrality = calculate_weighted_centrality(G)
        ...
        direct_answer = f"""
        Based on my analysis of the knowledge graph, the top 5 topics/entities in the dataset (by centrality) are:
        ...
        These represent the most central entities in the knowledge graph, with {G.number_of_nodes()} total entities 
        organized into {len(community_summaries)-1} communities.
        
        Entity types in the dataset include: 
        {', '.join([f"{t} ({c})" for t, c in sorted(entity_types.items(), key=lambda x: x[1], reverse=True)])}
        """
        
        debug("Direct answer generated for top topics.")
        return direct_answer.strip()
    
    # For general questions about dataset (that aren't specifically asking for top topics)
    general_question_patterns = [
        "what is the data about", 
        "what are the topics", 
    ...
    # For other questions, use the improved community filtering
    filtered_communities = filter_relevant_communities_improved(question, community_summaries, sent_transformer)
    
    # If no communities were found, return a helpful message
    if not filtered_communities:
        return "I couldn't find specific information to answer that question in the dataset."
    
    # Generate partial answers from each relevant community
    partials = []
...
def main():
    ...
    # Create Metadata Community 
    debug("Creating metadata community for general questions...")
    community_summaries = create_metadata_community(community_summaries, G_merged)
    debug("Metadata community added to summaries.")
    ...
    while True:
        ...
        if query.lower() == "exit":
            ...
            break
        # Process the query using Graph-RAG with Microsoft-inspired improvements
        answer = graph_rag_query_map_reduce_improved(query, community_summaries, summ_tokenizer, summ_model, sent_transformer, G_merged)
        print("\nBot:", answer)
