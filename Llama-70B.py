...
'''Reason: Use an internal chatbot based on the Llama 3 model instead of ChatGPT 
...
# --------------------------------------------------------------------------
# 1. Load Llama 3 - 70B Model (Multi-GPU Support)
# --------------------------------------------------------------------------
...
def load_llama_model():
    ...
    model = AutoModelForCausalLM.from_pretrained(
        "meta-llama/Meta-Llama-3-70B", token=HF_TOKEN,
        torch_dtype=dtype,
        device_map="auto"
    )
...
# --------------------------------------------------------------------------
# 5. Generate Response using Llama (Restrict Answers + Multi-GPU Optimization)
# --------------------------------------------------------------------------
...
def generate_response(query, tokenizer, model, embedding_model, collection, device):
    ...
    prompt = f"""
    You are an academic research assistant. Answer strictly based on the provided academic papers. 
    If the information is not available, state: *"I cannot find this information in my database."*
    ...
    with torch.no_grad():
        ...
        output = model.generate(**inputs, max_length=1024, do_sample=False, num_return_sequences=1, eos_token_id=tokenizer.eos_token_id)
