...
def hierarchical_merge_entities(entities, embeddings, threshold=0.9):
    """
    Clusters and merges similar entity names based on cosine similarity.
    """
    clustering = AgglomerativeClustering(
        n_clusters=None, distance_threshold=1 - threshold, linkage="average"
    )
    ...
    for i, label in enumerate(labels):
        ...
        merged_entities[label].append(entities[i])

    return merged_entities

def merge_similar_nodes(G, threshold=0.9):
    """
    Merges near-duplicate nodes in the knowledge graph using hierarchical clustering.
    """
    ...
    # Use hierarchical clustering to find groups of similar entities
    clusters = hierarchical_merge_entities(names, embeddings, threshold)

    G_copy = copy.deepcopy(G)
    for cluster_entities in clusters.values():
        if len(cluster_entities) > 1:
            main_entity = cluster_entities[0]  # Choose the first entity as the representative
            for entity in cluster_entities[1:]:
                if not G_copy.has_node(entity):
                    continue
                attr_main = G_copy.nodes[main_entity]
                attr_other = G_copy.nodes[entity]
                
                # Preserve missing attributes from merged nodes
                if "doc_idx" not in attr_main and "doc_idx" in attr_other:
                    attr_main["doc_idx"] = attr_other["doc_idx"]
                if "chunk_idx" not in attr_main and "chunk_idx" in attr_other:
                    attr_main["chunk_idx"] = attr_other["chunk_idx"]
                if "description" not in attr_main and "description" in attr_other:
                    attr_main["description"] = attr_other["description"]

                # Merge entity into main node
...
def generate_partial_answer(question, summary_text, tokenizer, model):
    """
    Generates a concise partial answer from a community summary.
    Ensures responses are relevant, structured, and academically appropriate.
    """
    system_instruction = """\
You are a research assistant specializing in academic summarization.
Extract key insights from the following academic summary that directly answer the user's question.
Ensure that:
- The response is **concise, structured, and academic**.
- It does **not include system instructions**.
- The response **directly answers the question**.
- Always **cite the source** of the information if available.
"""
    prompt = f"""
{system_instruction}
...
"""
    ...
    # Ensure we extract only the final answer
...
def combine_answers(question, partial_answers, tokenizer, model):
    """
    Merges multiple partial answers into a final, structured academic response.
    """
    if not partial_answers:
        return "No relevant information found."

    # Step 1: Remove redundant or similar answers
    filtered_answers = filter_redundant_answers(partial_answers, tokenizer, model)

    # Step 2: Format answers in a structured manner
    structured_parts = "\n\n".join([f"- {ans}" for _, ans in filtered_answers])

    system_instruction = """\
You are an AI academic assistant. Your task is to merge multiple research insights into a **coherent, structured response**.
Ensure that:
- The response is **organized, formal, and directly answers the question**.
- **Contradictory points are reconciled**.
- **Proper citations** are used when referencing academic papers.
"""
    prompt = f"""
{system_instruction}
...
def filter_relevant_communities(question, community_summaries, tokenizer, model):
    """
    Only select communities highly relevant to the user’s query.
    """
    query_embedding = get_embedding(question, tokenizer, model)
    community_texts = list(community_summaries.values())
    community_embeddings = np.array([get_embedding(text, tokenizer, model) for text in community_texts])
    
    sim_scores = cosine_similarity([query_embedding], community_embeddings)[0]
    selected_communities = {
        cid: summary for cid, (summary, score) in enumerate(zip(community_summaries.values(), sim_scores))
        if score > 0.75  # Only include communities with strong relevance
    }
...
def graph_rag_query_map_reduce(question, community_summaries, tokenizer, model):
    """
    1) Select relevant communities based on query similarity.
    2) Extract key insights from those communities.
    3) Merge partial insights into a final structured response.
    """
    # Step 1: Filter relevant communities
    filtered_communities = filter_relevant_communities(question, community_summaries, tokenizer, model)

    # Step 2: Generate partial answers from relevant communities
    partials = []
    for cid, summ_text in filtered_communities.items():
        ans = generate_partial_answer(question, summ_text, tokenizer, model)
        if ans.strip().lower().startswith("no relevant information"):
            continue
        partials.append((cid, ans))

    # Step 3: Merge partial answers into a final coherent response
